...
class Bootstrap {
	...
	public function halt_code(): void {
		...
		if ( ! defined( 'ADVADS_VERSION' ) ) {
			$advads_halt_notices[] = __( 'Advanced Ads - Slider', 'slider-ads' );
			add_action( 'all_admin_notices', [ $this, 'print_missing_notices' ] );
			add_action( 'after_plugin_row_' . plugin_basename( AAS_FILE ), [ $this, 'missing_row' ] );
			return;
		}
		...
		if ( version_compare( ADVADS_VERSION, '2.0.0', '<' ) ) {
			...
			add_action( 'after_plugin_row_' . plugin_basename( AAS_FILE ), [ $this, 'compatible_row' ] );
			return;
		}

		// Start it.
		add_action( 'advanced-ads-loaded', 'wp_advads_slider' );
	}
	...
	public function missing_row(): void {
		$this->print_row(
			__( '<strong>Advanced Ads - Slider</strong> requires the <strong>Advanced Ads free</strong> plugin to be installed and activated on your site.', 'slider-ads' )
	...
	public function compatible_row(): void {
		$this->print_row(
			sprintf(
				...
				__( 'Your version of <strong>Advanced Ads - Slider</strong> is incompatible with <strong>Advanced Ads %s</strong> and has been deactivated. Please update the plugin to the latest version.', 'slider-ads' ),
				ADVADS_VERSION
			)
		);
	}
	...
	public function print_missing_notices(): void {
		...
		$this->print_notices(
			__( 'Important Notice', 'slider-ads' ),
			__( 'Addons listed below requires the <strong><a href="https://wpadvancedads.com/?utm_source=advanced-ads&utm_medium=link&utm_campaign=activate-slider-ads" target="_blank">Advanced Ads</a></strong> plugin to be installed and activated on your site.', 'slider-ads' )
			. '&nbsp;' . $this->get_button(),
			$advads_halt_notices
		);
	...
	public function print_halt_notices(): void {
		...
		$this->print_notices(
			...
			sprintf(
				/* translators: %s: Plugin name */
				__( 'Your versions of the Advanced Ads addons listed below are incompatible with <strong>Advanced Ads %s</strong> and have been deactivated. Please update the plugin to the latest version.', 'slider-ads' ),
				ADVADS_VERSION
			),
			$advads_halt_notices
		);
	...
	private function print_notices( $title, $description, $notices ): void {
		...
		<div class="notice notice-error">
			...
			<h3><?php esc_html_e( 'The following addons are affected:', 'slider-ads' ); ?></h3>
			<ul>
	...
	private function get_button( $type = 'button-primary' ): string {
		...
		$text = __( 'Install Now', 'slider-ads' );
		...
		if ( isset( $plugins['advanced-ads/advanced-ads.php'] ) ) {
			...
			$text = __( 'Activate Now', 'slider-ads' );
		}
