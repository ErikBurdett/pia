...
if (!defined('ADL_LOG_RETENTION_DAYS')) {
    define('ADL_LOG_RETENTION_DAYS', 90);
}

// CONFIG: Log storage location under uploads
if (!defined('ADL_LOG_DIR')) {
    define('ADL_LOG_DIR', 'adl-activity');
}

// CONFIG: Error log subdirectory under ADL_LOG_DIR
if (!defined('ADL_ERROR_LOG_SUBDIR')) {
    define('ADL_ERROR_LOG_SUBDIR', 'errors');
}

// CONFIG: Disable noisy option logging (recommended)
if (!defined('ADL_LOG_OPTIONS')) {
    define('ADL_LOG_OPTIONS', false);
}

// CONFIG: Option prefixes to skip if option logging is enabled
if (!defined('ADL_OPTION_SKIP_PREFIXES')) {
    define('ADL_OPTION_SKIP_PREFIXES', [
        '_transient_',
        '_site_transient_',
        'cron',
        'rewrite_rules',
...
add_action('save_post', 'adl_safe_log_post_save', 20, 3);
add_action('transition_post_status', 'adl_safe_log_post_status_change', 10, 3);
add_action('deleted_post', 'adl_safe_log_post_deleted', 10, 1);
add_action('trashed_post', 'adl_safe_log_post_trashed', 10, 1);
add_action('untrashed_post', 'adl_safe_log_post_untrashed', 10, 1);
add_action('add_attachment', 'adl_safe_log_media_added', 10, 1);
add_action('edit_attachment', 'adl_safe_log_media_edited', 10, 1);
add_action('delete_attachment', 'adl_safe_log_media_deleted', 10, 1);
add_action('wp_insert_comment', 'adl_safe_log_comment_added', 10, 2);
add_action('wp_set_comment_status', 'adl_safe_log_comment_status', 10, 2);
add_action('switch_theme', 'adl_safe_log_theme_switch', 10, 3);
add_action('activated_plugin', 'adl_safe_log_plugin_activated', 10, 2);
add_action('deactivated_plugin', 'adl_safe_log_plugin_deactivated', 10, 2);
add_action('wp_update_nav_menu', 'adl_safe_log_menu_updated', 10, 1);
add_action('user_register', 'adl_safe_log_user_created', 10, 1);
add_action('deleted_user', 'adl_safe_log_user_deleted', 10, 1);
add_action('profile_update', 'adl_safe_log_user_updated', 10, 2);
add_action('updated_option', 'adl_safe_log_option_updated', 10, 3);
add_action('added_option', 'adl_safe_log_option_added', 10, 2);
add_action('deleted_option', 'adl_safe_log_option_deleted', 10, 1);
...
function adl_safe_call($callable, $args) {
    static $error_counts = [];
    try {
        call_user_func_array($callable, $args);
    } catch (Throwable $e) {
        $key = is_string($callable) ? $callable : 'callable';
        if (!isset($error_counts[$key])) {
            $error_counts[$key] = 0;
        }
        $error_counts[$key]++;
        if ($error_counts[$key] <= 3) {
            adl_log_error_event($key, $e);
        }
    }
}

function adl_safe_log_post_save($post_id, $post, $update) {
    adl_safe_call('adl_log_post_save', func_get_args());
}

function adl_safe_log_post_status_change($new_status, $old_status, $post) {
    adl_safe_call('adl_log_post_status_change', func_get_args());
}

function adl_safe_log_post_deleted($post_id) {
    adl_safe_call('adl_log_post_deleted', func_get_args());
}

function adl_safe_log_post_trashed($post_id) {
    adl_safe_call('adl_log_post_trashed', func_get_args());
}

function adl_safe_log_post_untrashed($post_id) {
    adl_safe_call('adl_log_post_untrashed', func_get_args());
}

function adl_safe_log_media_added($post_id) {
    adl_safe_call('adl_log_media_added', func_get_args());
}

function adl_safe_log_media_edited($post_id) {
    adl_safe_call('adl_log_media_edited', func_get_args());
}

function adl_safe_log_media_deleted($post_id) {
    adl_safe_call('adl_log_media_deleted', func_get_args());
}

function adl_safe_log_comment_added($comment_id, $comment) {
    adl_safe_call('adl_log_comment_added', func_get_args());
}

function adl_safe_log_comment_status($comment_id, $status) {
    adl_safe_call('adl_log_comment_status', func_get_args());
}

function adl_safe_log_theme_switch($new_name, $new_theme, $old_theme) {
    adl_safe_call('adl_log_theme_switch', func_get_args());
}

function adl_safe_log_plugin_activated($plugin, $network_wide) {
    adl_safe_call('adl_log_plugin_activated', func_get_args());
}

function adl_safe_log_plugin_deactivated($plugin, $network_wide) {
    adl_safe_call('adl_log_plugin_deactivated', func_get_args());
}

function adl_safe_log_menu_updated($menu_id) {
    adl_safe_call('adl_log_menu_updated', func_get_args());
}

function adl_safe_log_user_created($user_id) {
    adl_safe_call('adl_log_user_created', func_get_args());
}

function adl_safe_log_user_deleted($user_id) {
    adl_safe_call('adl_log_user_deleted', func_get_args());
}

function adl_safe_log_user_updated($user_id, $old_user_data) {
    adl_safe_call('adl_log_user_updated', func_get_args());
}

function adl_safe_log_option_updated($option, $old_value, $value) {
    adl_safe_call('adl_log_option_updated', func_get_args());
}

function adl_safe_log_option_added($option, $value) {
    adl_safe_call('adl_log_option_added', func_get_args());
}

function adl_safe_log_option_deleted($option) {
    adl_safe_call('adl_log_option_deleted', func_get_args());
}
...
function adl_compute_text_diff($old, $new) {
    ...
    if (strpos($old, "\n") !== false || strpos($new, "\n") !== false) {
        $diff = adl_simple_line_diff($old, $new);
        return adl_limit_text($diff);
    }
    $diff = adl_minimal_text_change($old, $new);
    return adl_limit_text($diff);
}
...
function adl_simple_line_diff($old, $new) {
...
function adl_minimal_text_change($old, $new) {
    $old_len = strlen($old);
    $new_len = strlen($new);
    $prefix = 0;
    ...
    $old_change = substr($old, $prefix, $old_len - $prefix - $suffix);
    $new_change = substr($new, $prefix, $new_len - $prefix - $suffix);
...
function adl_log_error_event($handler, Throwable $e) {
    $user = wp_get_current_user();
    $event = [
        'ts' => gmdate('c'),
        'site_id' => get_current_blog_id(),
...
function adl_get_error_log_dir($site_id = null) {
    $site_id = is_null($site_id) ? get_current_blog_id() : (int) $site_id;
    $uploads = adl_get_upload_dir_for_site($site_id);
    return trailingslashit($uploads['basedir']) . ADL_LOG_DIR . '/' . ADL_ERROR_LOG_SUBDIR . '/site-' . $site_id;
}
...
function adl_write_event($event) {
    ...
    adl_write_ndjson($dir, $event);
}

function adl_write_error_event($event) {
    $site_id = (int) $event['site_id'];
    $dir = adl_get_error_log_dir($site_id);
    if (!wp_mkdir_p($dir)) {
        error_log('[AvadaDiff] Unable to create error log directory: ' . $dir);
        return;
    }
    adl_write_ndjson($dir, $event);
}

function adl_write_ndjson($dir, $event) {
...
function adl_render_network_page() {
    ...
    adl_handle_error_csv_export($start_date, $end_date, $site_id);
    adl_handle_clear_logs();
    ...
    $error_export_url = add_query_arg(
        [
            'page' => 'adl-activity-monitor',
            'start_date' => $start_date,
            'end_date' => $end_date,
            'site_id' => $site_id,
            'adl_export' => 'error_csv',
        ],
        network_admin_url('admin.php')
    );
    $error_export_url = wp_nonce_url($error_export_url, 'adl_export_error_csv');
    ...
    $clear_url = wp_nonce_url($clear_url, 'adl_clear_logs');
    echo '<p><a class="button" href="' . esc_url($export_url) . '">Export CSV</a> ';
    echo '<a class="button" href="' . esc_url($error_export_url) . '">Export Error CSV</a> ';
    echo '<a class="button button-secondary" href="' . esc_url($clear_url) . '" onclick="return confirm(\'Clear all activity logs? This cannot be undone.\')">Clear Logs</a></p>';
    ...
    $error_events = adl_query_error_events($start_date, $end_date, $site_id);
    echo '<h2>Handler Errors</h2>';
    echo '<table class="widefat striped"><thead><tr>';
    echo '<th>Date</th><th>Site</th><th>Handler</th><th>User</th><th>Message</th><th>Location</th><th>Details</th>';
    echo '</tr></thead><tbody>';
    if (empty($error_events)) {
        echo '<tr><td colspan="7">No handler errors in this time window.</td></tr>';
    } else {
        foreach ($error_events as $event) {
            $json = wp_json_encode($event, JSON_PRETTY_PRINT);
            $location = '';
            if (!empty($event['file']) || !empty($event['line'])) {
                $location = ($event['file'] ?? '') . ':' . ($event['line'] ?? '');
            }
            echo '<tr>';
            echo '<td>' . esc_html($event['ts'] ?? '') . '</td>';
            echo '<td>' . esc_html($event['site_id'] ?? '') . '</td>';
            echo '<td>' . esc_html($event['handler'] ?? '') . '</td>';
            echo '<td>' . esc_html($event['user_login'] ?? '') . ' (' . esc_html($event['user_id'] ?? '') . ')</td>';
...
function adl_handle_error_csv_export($start_date, $end_date, $site_id) {
    if (!isset($_GET['adl_export']) || $_GET['adl_export'] !== 'error_csv') {
        return;
    }
    if (!isset($_GET['_wpnonce']) || !wp_verify_nonce($_GET['_wpnonce'], 'adl_export_error_csv')) {
        wp_die('Invalid error export request.');
    }

    $events = adl_query_error_events($start_date, $end_date, $site_id, 0);

    $filename = sprintf(
        'activity-errors-%s-to-%s.csv',
        preg_replace('/[^0-9\-]/', '', $start_date),
        preg_replace('/[^0-9\-]/', '', $end_date)
    );
    ...
    $out = fopen('php://output', 'w');
    fputcsv($out, ['ts', 'site_id', 'handler', 'user_login', 'user_id', 'message', 'file', 'line']);
    foreach ($events as $event) {
        fputcsv($out, [
            $event['ts'] ?? '',
            $event['site_id'] ?? '',
...
function adl_handle_clear_logs() {
    if (!isset($_GET['adl_clear_logs']) || $_GET['adl_clear_logs'] !== '1') {
        return;
    }
    if (!isset($_GET['_wpnonce']) || !wp_verify_nonce($_GET['_wpnonce'], 'adl_clear_logs')) {
        wp_die('Invalid clear logs request.');
    }
    adl_clear_all_logs();
    wp_safe_redirect(network_admin_url('admin.php?page=adl-activity-monitor'));
    exit;
}

function adl_clear_all_logs() {
    $sites = get_sites(['fields' => 'ids']);
    $site_ids = is_array($sites) ? $sites : [];
    foreach ($site_ids as $sid) {
        $dir = adl_get_log_dir($sid);
        adl_delete_dir($dir);
        $error_dir = adl_get_error_log_dir($sid);
        adl_delete_dir($error_dir);
    }
}

function adl_delete_dir($dir) {
    if (!is_dir($dir)) {
        return;
    }
    $items = scandir($dir);
...
function adl_query_error_events($start_date, $end_date, $site_id = 0, $limit = 200) {
    $events = [];
    $start = strtotime($start_date . ' 00:00:00 UTC');
    $end = strtotime($end_date . ' 23:59:59 UTC');
    if (!$start || !$end || $start > $end) {
    ...
    for ($ts = $start; $ts <= $end; $ts = strtotime('+1 day', $ts)) {
        $date = gmdate('Y-m-d', $ts);
        foreach ($site_ids as $sid) {
            $file = adl_get_error_log_dir($sid) . '/' . $date . '.ndjson';
            if (!file_exists($file)) {
                continue;
            }
            $handle = @fopen($file, 'r');
